...
class Grayscale_Module(object):
    ...
    def set_reference(self, ref: list) -> list:
    ...
    def get_status(self, datas: list = None):
        """
        Read line status
        ...
        datas = [0 if data > self._reference[i] else 1 for i, data in enumerate(datas)]
        
        if datas == [1,1,1] or datas == [0,1,0]:
            return 'forward'
        elif datas == [0,1,1] or datas == [0,0,1]:
            return 'right'
        elif datas == [1,1,0] or datas == [1,0,0]:
            return 'left'
        else:
            return 'out'
